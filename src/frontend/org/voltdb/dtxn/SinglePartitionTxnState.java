...
public class SinglePartitionTxnState extends TransactionState {
                                   ...
                                   TransactionInfoBaseMessage task)
    {
        ...
        m_isCVoltDBExecution = ((InitiateTaskMessage)task).isCVoltDBExecution();
    }
    ...
    public boolean doWork(boolean recovering) {
        ...
        if (!m_done) {
            ...
            m_response = m_site.processInitiateTask(this, m_task);
            
            if (m_response.shouldCommit() == false) {
                m_needsRollback = true;
            }

            m_done = true;
        
            // GWW: set m_prepared true here to be consistant with multi-site txn
            m_prepared = true;
        }
