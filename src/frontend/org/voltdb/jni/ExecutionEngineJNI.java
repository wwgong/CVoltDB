...
public class ExecutionEngineJNI extends ExecutionEngine {
      ...
      throws EEException
    {
        ...
        final int errorCode = nativeExecutePlanFragment(pointer, planFragmentId, outputDepId, inputDepId,
                                                        txnId, lastCommittedTxnId, executedPlanNodes, isCVoltDB, undoToken);
        try {
            checkErrorCode(errorCode);
    ...
    public VoltTable[] executeQueryPlanFragmentsAndGetResults(
            ...
            final long txnId, final long lastCommittedTxnId, final long undoToken
            , final long startBatch, final long startPlanNode, final boolean isCVoltDB) throws EEException {
        ...
        fsForParameterSet.clear();
        for (int i = (int)startBatch; i < batchSize; ++i) {
            try {
	...
	@Override
	public boolean release2UndoToken(final long txnId, final long beginUndoToken, final long endUndoToken) {
		return nativeRelease2UndoToken(pointer, txnId, beginUndoToken, endUndoToken);
	}

	@Override
	public boolean undo2UndoToken(final long txnId, final long beginUndoToken, final long endUndoToken) {
        return nativeUndo2UndoToken(pointer, txnId, beginUndoToken, endUndoToken);
	}
}
