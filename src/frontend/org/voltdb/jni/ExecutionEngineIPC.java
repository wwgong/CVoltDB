...
//        }
    ...
    public DependencyPair executePlanFragment(final long planFragmentId, final int outputDepId,
            ...
            final long lastCommittedTxnId, final long executedPlanNodes, final boolean isCVoltDB, final long undoToken)
            throws EEException {
        // big endian, not direct
        final FastSerializer fser = new FastSerializer();
        try {
        ...
        m_data.putLong(lastCommittedTxnId);
        // GWW
        m_data.putLong(executedPlanNodes);
        m_data.putLong(undoToken);
    ...
    public VoltTable[] executeQueryPlanFragmentsAndGetResults(
            final long[] planFragmentIds, final int numFragmentIds,
            final ParameterSet[] parameterSets, final int numParameterSets, 
            final long txnId, final long lastCommittedTxnId, final long undoToken, 
            long startBatch, long startPlanNode, boolean isCVoltDB) throws EEException {
