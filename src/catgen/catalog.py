...
def javatypify( x ):
    ...
    #GWW
    elif x[-2] == '[' and x[-1] == ']': return 'ArrayList<%s>' % javaobjectify( x.rstrip('[]') )
...
def genjava( classes, prepath, postpath, package ):
    ...
    for cls in classes:
        ...
        for field in cls.fields:
            ...
            # GWW
            elif fname == 'partitionparams':
                write( interp( '    $ftype m_$fname = new $ftype();', locals() ) )
            elif field.type[-1] == '?':
...
def cpptypify( x ):
    ...
    #GWW
    elif x[-2] == '[' and x[-1] == ']': return 'std::vector<%s>' % cpptypify(x.rstrip('[]'))
    else: raise Exception( 'bad type: ' + x )
...
def gencpp( classes, prepath, postpath ):
    ...
    for cls in reversed( classes ):
        ...
        for field in cls.fields:
            ...
            # GWW
            elif field.type[-2] == '[' and field.type[-1] == ']':
        		write ( interp( '    const $ftype $pubname() const;', locals() ) )
            else:
