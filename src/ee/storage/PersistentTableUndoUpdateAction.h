...
public:
    ...
    inline PersistentTableUndoUpdateAction(
            ...
            voltdb::Pool *pool)
        : m_oldTuple(oldTuple), m_table(table), m_revertIndexes(false), m_ignoreUndoAction(false), m_tupleJournal(NULL)
    {
        /*
    ...
    // GWW: when a CFE happens, undo the action and set this to be true
    // then when release/undo, we only perform change when this field is false
    inline void ignoreUndoAction() {
    	m_ignoreUndoAction = true;
    }
...
private:
    ...
    // GWW: this is set to be true if an exception occurs during execution
    // then we should ignore all operations needed for release/undo
    // because we already clean up before we report the exception
    bool m_ignoreUndoAction;
