...
void PersistentTableUndoDeleteAction::undo() {
    if(ExecutorContext::getExecutorContext()->isCVoltDBExecution()) {
        // GWW: when escrow enabled, if abort, no re-insert of index entries
    	// needed because we did not delete this tuple, so just clean journal
...
void PersistentTableUndoDeleteAction::release() {
	...
	// GWW: now delete them from all indexes
    if(ExecutorContext::getExecutorContext()->isCVoltDBExecution()) {
        m_table->cleanTupleJournal(m_tupleJournal);
   		m_table->deleteFromAllIndexes(&tuple);
    }

    tuple.setPendingDeleteOnUndoReleaseFalse();
    m_table->m_tuplesPinnedByUndo--;
    m_table->m_usedTupleCount--;
