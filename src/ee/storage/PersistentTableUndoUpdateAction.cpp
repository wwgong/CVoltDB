...
void PersistentTableUndoUpdateAction::undo() {
	...
	VOLT_TRACE("GWWW - undo: ignoreUndoAction = %d, revertIndexes = %d\n.",
			m_ignoreUndoAction, m_revertIndexes );

	if(!m_ignoreUndoAction) {
		...
		m_table->updateTupleForUndo(m_oldTuple, tupleInTable, m_revertIndexes,m_tupleJournal);
...
void PersistentTableUndoUpdateAction::release() {
	...
	VOLT_TRACE("GWWW - release: ignoreUndoAction = %d, revertIndexes = %d\n.",
			m_ignoreUndoAction, m_revertIndexes );
	...
	if(!m_ignoreUndoAction) {
		...
		if(ExecutorContext::getExecutorContext()->isCVoltDBExecution()) {
			// Now we can do cleanup job until the last undoAction
			// using saved journalPtr

			m_table->cleanTupleJournal(m_tupleJournal);
		}
	}
}
