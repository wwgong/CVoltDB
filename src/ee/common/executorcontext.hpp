...
class ExecutorContext {
  public:
    ...
    void setCVoltDBExecution(bool cVoltDBExecution) {
    	m_cVoltDBExecution = cVoltDBExecution;
    }

    bool isCVoltDBExecution() {
    	return m_cVoltDBExecution;
    }

    void setCheckEscrowOutOfBound(bool isCheck) {
    	m_checkEscrowOutOfBound = isCheck;
    }

    bool isCheckEscrowOutOfBound() {
    	return m_checkEscrowOutOfBound;
    }

    void setThrowEscrowOutOfBoundException(bool isThrow) {
    	m_throwEscrowOutOfBoundException = isThrow;
    }

    bool isThrowEscrowOutOfBoundException() {
    	return m_throwEscrowOutOfBoundException;
    }

#if defined(DEBUG) || defined(_DEBUG) || defined(_DEBUG_)
    std::deque<UndoQuantum*> getTxnUndoQuantums(int64_t txnid) {
//    	std::map<int64_t, std::deque<UndoQuantum*> >::iterator itr = m_txnUndoQuantumsMap.find(txnid);
//    	if(itr != m_txnUndoQuantumsMap.end())
    		return m_txnUndoQuantumsMap.find(txnid)->second;
//    	else
//    		return NULL;
    }
#endif
  ...
  public:
    ...
    // GWW
    bool m_cVoltDBExecution;
    bool m_checkEscrowOutOfBound;
    bool m_throwEscrowOutOfBoundException;
    // for debug purpose
//    std::vector<int64_t> m_preparedTxnIds;
    int64_t m_releasingTxnId;
#if defined(DEBUG) || defined(_DEBUG) || defined(_DEBUG_)
    boost::unordered_map<int64_t, std::deque<UndoQuantum*> > m_txnUndoQuantumsMap;
#endif
};
