...
public:
    ...
    // GWW: escrow column
    static TupleSchema* createTupleSchema(const std::vector<ValueType> columnTypes, const std::vector<int32_t> columnSizes, const std::vector<bool> allowNull, const std::vector<bool> isEscrowColumn, bool allowInlinedStrings);
    ...
    // GWW
    inline bool isEscrowColumn(const int index) const;
...
private:
    ...
    struct ColumnInfo {
        ...
        bool escrowCol;		// GWW: whether this column is an escrow-type column
    };
    ...
    // GWW
    void setColumnMetaData(uint16_t index, ValueType type, const int32_t length, bool allowNull, bool escrowColumn, uint16_t &uninlinedObjectColumnIndex);
...
// GWW
inline bool TupleSchema::isEscrowColumn(const int index) const {
	assert(index < m_columnCount);
	const ColumnInfo *columnInfo = getColumnInfo(index);
	return columnInfo->escrowCol;
}
