...
public:
    ...
    inline void undo() {
    	...
    	printf("GWWW: UndoQuantum undo - # of undo actions: %lu\n", m_undoActions.size());
#endif
    ...
    inline void release() {
#if defined(DEBUG) || defined(_DEBUG) || defined(_DEBUG_)
    	printf("GWWW: UndoQuantum release - # of undo actions: %lu\n", m_undoActions.size());
#endif
    	if(m_undoActions.size() > 0) {
			for (std::vector<UndoAction*>::reverse_iterator i = m_undoActions.rbegin();
				 i != m_undoActions.rend(); i++) {
...
#if defined(DEBUG) || defined(_DEBUG) || defined(_DEBUG_)
				UndoAction *p = dynamic_cast<UndoAction*>(*i);
				assert(p);
				printf("EON applying undoaction %lu (release) for txn %lu \n", (long)p, m_txnId);
				(*i)->debug();
#endif
    ...
    // virtual removed--this fails where non-vitual methods work  (why is this marked  virtual?)
    inline Pool* getDataPool() {
        return m_dataPool;
    }
